...
const ScenarioCard = ( { type, username, setSelected=undefined, selectedId="", fieldName="" } ) => {
    ...
    useEffect(() => {
        const ScenarioCharacterEntrys = async () => {
            setScenario(await GetEntries("Scenarios", username))
        }
    ...
    <div className='flex flex-col w-[95%] max-w-[1200px] rounded-lg min-h-[250px]'>
        ...
        <div className=' flex flex-col md:flex-row gap-4 items-center'>
            ...
            {(Scenario.length >= 1 && type != "edit") &&
                <button onClick={() => navigate("/CreateScenario")} className={`cursor-pointer font-Comfortaa pr-2 pl-2 rounded-md font-bold flex-none h-[50px] ${(type  == "display") ? ' bg-slate-900 ' : ' bg-slate-800 '}`}>Create Scenario</button>
            }
            </div>
                ...
                {Scenario.map((element, index) => (
                    <div className={` min-h-[200px] min-w-[200px] rounded-md flex flex-col items-center mt-5 mb-2 ${(type  == "display") ? ' bg-slate-900 ' : ' bg-slate-800 '}`} key={index}>
                        ...
                        <div className=' flex gap-2 mt-auto mb-4'>
                            ...
                            {(type == "display") &&
                                <>
                                    <button onClick={() => navigate("/EditScenario/:"+element["_id"])} className=' btn text-white bg-purple-700 hover:bg-purple-700'>Edit</button>
                                    <button onClick={() => DeleteEntry("Scenarios", element["_id"])} className=' btn text-white bg-red-800 hover:bg-red-800'>Delete</button>
                                </>
                            }
